#> core:evaluation/alphabeta-5/1
# @within core:evaluation/alphabeta-5/*

# $t10: rootの各子ノードでのevaluationの最小値保存用スコア

# tree[0].nextが空であるかによる分岐
execute if data storage othello:tree tree[0].next[0] run function core:evaluation/alphabeta-5/1-0
execute unless data storage othello:tree tree[0].next[0] run function core:evaluation/alphabeta-5/1-1
...
execute if score $t14 Temp matches 0 run function core:evaluation/alphabeta-5/1