#> core:evaluation/minmax/2
# @within core:evaluation/minmax/*

# tree[0].nextが空であるかによる分岐
execute if data storage othello:tree tree[0].next[0].next[0] run function core:evaluation/minmax/2-0
execute unless data storage othello:tree tree[0].next[0].next[0] run function core:evaluation/minmax/2-1
...
execute if score $t14 Temp matches 0 run function core:evaluation/minmax/2