...
execute store result score $t11 Temp run data get storage othello: candidate_count
execute store result storage othello:tree tree[0].next[0].player int 1 run scoreboard players get $waiting Settings

function core:control/change_player/main
function core:tree/initialize/3
function core:control/change_player/main
...
execute unless score $t10 Temp matches ..0 run function core:tree/initialize/2
...
#execute if score $t0 Temp matches 1.. run function core:tree/initialize/2