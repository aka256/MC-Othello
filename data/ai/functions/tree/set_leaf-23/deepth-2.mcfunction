...
# candidateが生成されたかによる分岐
  ...
  execute if score $t8 Temp matches 1 run function ai:tree/set_leaf-23/deepth-3
  # candidateが生成されなかったとき、deepth-2_skipを実行
  execute if score $t8 Temp matches 0 run function ai:tree/set_leaf-23/deepth-2_skip
...
# リストtree[0].nextの回転
  ...
  execute if score $t5 Temp matches ..0 run function ai:tree/set_leaf-23/deepth-1_tail
  execute if score $t5 Temp matches 1.. run function ai:tree/set_leaf-23/deepth-2